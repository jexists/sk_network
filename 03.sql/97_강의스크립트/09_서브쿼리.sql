...
-- '적어도 한 명의 급여보다 더 높은' 급여를 받는 직원 조회
...
-- '영업부'에 속한 직원이 실제로 존재하는 확인
-- EXISTS는 서브쿼리 결과가 1행이라도 있으면 TRUE를 반환
SELECT EXISTS(
	SELECT 1
	FROM employee2
...
-- 각 직원(e1)에 대해 같은 부서 안에 나보다 월급이 더 높은 사람(e2)이 있는지 확인
...
SELECT
	...
	(	-- 서브 쿼리가 그 부서의 평균 급여 계산
		SELECT AVG(salary)
		FROM employee2
		WHERE department = e.department  	-- 바깥 쿼리의 department를 기준으로 
	) AS avg_salary_per_dept
FROM employee2 e; 


-- 7. FROM 절의 서브쿼리 = 인라인 뷰
