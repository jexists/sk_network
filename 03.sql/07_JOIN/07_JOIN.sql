...
-- join
-- 두 테이블을 공통된 값(조건)으로 연결해서 조회 하는 방법
...
-- 두 테이블을 inner join하고 '딱지남'을 제외한 name 컬럼 출력
SELECT sg1.name
FROM squid_game1 AS sg1
INNER JOIN squid_game2 sg2
	ON sg1.CAST = sg2.CAST
...
-- 주의 : OUTER JOIN에서 조건을 WHERE에 잘못 두면 누락 데ㅔ이터가 사라질 수 있다.

-- 시즌2에만 있는 cast, name 조회
SELECT sg2.CAST, sg2.name
FROM squid_game2 AS sg2
